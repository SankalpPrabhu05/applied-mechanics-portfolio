...
for t = 2:n_s+1
    ...
    utr = (u_old)+(dt.*v_old)+((dt^2)/2)*MLinv*(F-S*u_old);
    utr(fixdof)=0;
    ...
    while k ==0 || bool
        ...
        if nC~=0
            ...
            B_eff = 0.5*dt^2*C_c*MLinv*C_c';
            rhs = G_c+C_c*(utr-u_old);
            lambda = B_eff\rhs;
            u_n = utr - 0.5*(dt^2)*MLinv*C_c'*lambda;
            u_n(fixdof)=0;
        ...
        if ~isempty(lambda)
            ...
            new_active_local=false(num_candidate,1);
        end
...
% Smoothened Force History

figure
plot(T_s, smoothdata(force_hist, 'movmean', 100))
xlabel ('Indentation [in mm]')
ylabel ('Force [in N]')
title('Smoothened Total Contact Force v/s Indentation')
...
axis equal;hold on;
    theta = linspace(-pi/6, -pi/2, 200); 
    xc = x_c(1); yc = x_c(2);% bottom 180 degrees
    xarc = xc + R*cos(theta);
    yarc = yc + R*sin(theta);
